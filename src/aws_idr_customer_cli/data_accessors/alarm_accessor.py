...
class AlarmAccessor:
    ...
    def _handle_error(self, error: Exception, operation: str) -> None:
        ...
        if error_code == "AccessDeniedException" or error_code == "AccessDenied":
            raise PermissionError(f"Access denied to CloudWatch API: {str(error)}")
        elif (
    ...
    @track(dimensions={"Service": "CloudWatch", "ApiOperation": "DescribeAlarms"})
    ...
    @track(dimensions={"Service": "CloudWatch", "ApiOperation": "PutMetricAlarm"})
    @retry(exceptions=ClientError, tries=MAX_RETRIES, delay=1, backoff=2, logger=None)
    ...
    @track(dimensions={"Service": "CloudWatch", "ApiOperation": "ListMetrics"})
    @retry(exceptions=ClientError, tries=MAX_RETRIES, delay=1, backoff=2, logger=None)
