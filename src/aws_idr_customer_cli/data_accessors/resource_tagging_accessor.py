...
class ResourceTaggingAccessor:
    ...
    def _handle_error(self, error: Exception, operation: str) -> None:
        ...
        if self._consecutive_failures >= self.CIRCUIT_BREAKER_THRESHOLD:
            self.logger.error(
                f"Circuit breaker triggered after {self._consecutive_failures} "
                f"consecutive failures calling {operation}"
            )
            raise RuntimeError(
                f"Circuit breaker open: Too many consecutive failures calling {operation}"
            )
        ...
        if error_code == "AccessDeniedException" or error_code == "AccessDenied":
            raise PermissionError(
                f"Access denied to Resource Groups Tagging API: {str(error)}"
            )
        elif (
            error_code == "InvalidParameterException" or error_code == "ValidationError"
        ):
    ...
    def _reset_circuit_breaker(self) -> None:
        """Reset the circuit breaker counter after successful API call."""
        self._consecutive_failures = 0
    ...
    def get_resources(
        ...
        try:
            ...
            self._handle_error(exception, "get_resources")
            raise
    ...
    @track(
        dimensions={"Service": "ResourceGroupsTaggingAPI", "ApiOperation": "GetTagKeys"}
    )
    @retry(exceptions=ClientError, tries=MAX_RETRIES, delay=1, backoff=2, logger=None)
