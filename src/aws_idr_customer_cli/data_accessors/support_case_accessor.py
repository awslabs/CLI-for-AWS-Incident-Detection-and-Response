...
class SupportCaseAccessor:
    ...
    def _handle_error(self, error: Exception, operation: str) -> None:
        ...
        if error_code == "AccessDeniedException" or error_code == "AccessDenied":
            raise PermissionError(f"Access denied to Support API: {str(error)}")
        elif (
    ...
    @track(dimensions={"Service": "Support", "ApiOperation": "CreateCase"})
    ...
    @track(dimensions={"Service": "Support", "ApiOperation": "AddAttachmentsToSet"})
    @retry(exceptions=ClientError, tries=MAX_RETRIES, delay=1, backoff=2, logger=None)
    ...
    @track(dimensions={"Service": "Support", "ApiOperation": "DescribeCases"})
    @retry(exceptions=ClientError, tries=MAX_RETRIES, delay=1, backoff=2, logger=None)
    def describe_cases(
        self,
    ...
    @track(dimensions={"Service": "Support", "ApiOperation": "AddCommunicationToCase"})
    @retry(exceptions=ClientError, tries=MAX_RETRIES, delay=1, backoff=2, logger=None)
    def add_communication_to_case(
        ...
        """Add communication to an AWS Support Case"""
        try:
