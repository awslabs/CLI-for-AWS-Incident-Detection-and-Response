...
from aws_idr_customer_cli.utils.constants import (
    LAMBDA_EDGE_SCAN_REGION_DENYLIST,
...
class CloudWatchMetricsAccessor(BaseAccessor):
    ...
    def find_regions_with_lambda_metrics(
        ...
        concurrently. Regions in LAMBDA_EDGE_SCAN_REGION_DENYLIST are skipped
        (regional outages with hung endpoints). Each region uses an aggressive
        connect/read timeout so a single unreachable endpoint cannot stall
        the whole scan.
        ...
        skipped = [r for r in regions if r in LAMBDA_EDGE_SCAN_REGION_DENYLIST]
        regions_to_scan = [
            r for r in regions if r not in LAMBDA_EDGE_SCAN_REGION_DENYLIST
        ]

        if skipped:
            self.logger.info(
                f"Skipping {len(skipped)} region(s) with known endpoint "
                f"reachability issues: {skipped}"
            )
