...
from aws_idr_customer_cli.utils.telemetry.decorator import track
...
class AlarmService:
    ...
    @track(namespace="AWS/IDR/CLI/Alarms")
    ...
    def alarm_creation_object_to_recommendation(
        ...
        Args:
            creation_object: AlarmCreation object

        Returns:
            AlarmRecommendation object
        """
        # Extract the template alarm name by removing the resource ID suffix
        stored_alarm_name = alarm.alarm_configuration.alarm_name
        resource_id = extract_resource_id_from_arn(alarm.resource_arn.arn)

        expected_suffix = f"-{resource_id}"
        if not stored_alarm_name.endswith(expected_suffix):
            raise ValueError(
                f"Alarm '{stored_alarm_name}' does not end with expected suffix '{expected_suffix}'"
            )

        template_alarm_name = stored_alarm_name[: -len(expected_suffix)]

        # Generate all alarm configurations for this resource
        alarm_configs = self.alarm_recommendation_service.generate_alarm_configurations(
            [alarm.resource_arn]
        )

        # Find the specific configuration that matches the template alarm name
        matching_config = None
        for config in alarm_configs:
            if config.get("alarm_name") == template_alarm_name:
                matching_config = config
                break

        if not matching_config:
            raise ValueError(
                f"Could not find alarm configuration for template alarm name: {template_alarm_name}"
            )

        # Convert config back to AlarmRecommendation
        recommendations = self._convert_configs_to_recommendations([matching_config])

        if len(recommendations) != 1:
            raise ValueError(
                f"Failed to convert configuration to recommendation for alarm: {stored_alarm_name}"
            )

        # Update with the original selection state
    ...
    def alarm_creation_objects_to_recommendations(
        ...
        return [self.alarm_creation_object_to_recommendation(alarm) for alarm in alarms]
