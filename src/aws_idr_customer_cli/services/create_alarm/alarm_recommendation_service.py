...
class AlarmRecommendationService:
    """Service for loading and processing alarm templates for create-alarm command integration."""
    ...
    def _get_service_type_from_arn(self, arn: str) -> Optional[str]:
        ...
        try:
            ...
            if service_name == "elasticloadbalancing":
                mapped_service = AwsServices.ALB.value
            elif (
                service_name == "ec2"
                and hasattr(parsed_arn, "resource_type")
                and parsed_arn.resource_type == "vpn-connection"
            ):
                mapped_service = AwsServices.VPN.value
            else:
                service_enum = ServiceConfigManager.get_service_enum(service_name)
                if not service_enum:
                    return None
    ...
    def _create_alarm_configuration(
        ...
        """Create alarm configuration from template and resource."""
        try:
    ...
    def populate_template_with_resource(
        ...
        if isinstance(dimensions, list):
            self._replace_dimension_placeholders(
        ...
        if service == "s3":
            ...
            if resource.startswith("/restapis/"):
                ...
                if len(parts) >= 3 and "api_id" in extraction_rules:
                    identifiers[extraction_rules["api_id"]] = parts[2]
