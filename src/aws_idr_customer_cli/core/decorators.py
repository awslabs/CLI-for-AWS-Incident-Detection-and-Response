...
from aws_idr_customer_cli.utils.telemetry.command_tracker import CommandTracker
    ...
    def decorator(cls: CommandClass) -> CommandClass:
        ...
        def wrapped_execute(
            ...
            **kwargs: Dict[str, Any],
            ...
            telemetry = injector.get(Telemetry)

            # Determine command path (main command & subcommand)
            cmd_path = ctx.command_path.split()
            cmd = cmd_path[1] if len(cmd_path) > 1 else name
            sub_cmd = cmd_path[2] if len(cmd_path) > 2 else None
            with CommandTracker(
                telemetry,
                command=cmd,
                subcommand=sub_cmd,
                ...
                flags={
                    ...
                    **kwargs,  # Include command-specific params
                },
            ):
