...
from aws_idr_customer_cli._version import __version__
...
class CommandRegistry:
    ...
    def create_cli(self) -> click.Group:
        ...
        # Get the command tracker to create our Click group
        command_tracker = self.injector.get(CliCommandTracker)

        # Create the CLI group with telemetry enabled
        cli_group = cast(
            click.Group,
            command_tracker.create_cli_group(
                name="idrcli",
                help_text="CLI application with auto-discovered commands.",
            ),
        )
        ...
        cli_group = click.version_option(version=__version__, message="%(version)s")(
            cli_group
        )
            ...
            ctx.obj.update(
                {
                    ...
                    "verbose": verbose,
                    "debug": debug,
                    "telemetry": self.injector.get(Telemetry),
                }
            )
